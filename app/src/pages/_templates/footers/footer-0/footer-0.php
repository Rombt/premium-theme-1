<?php
/**
 * Footer 0 template.
 *
 * @package Premium_Theme_1
 */

defined( 'ABSPATH' ) || exit;
?>

<div class="rmbt-full-width _rmbt-footer-0-full-width">
	<section class="rmbt-container _rmbt-footer-0">
		<div class="_rmbt-footer-0__row _rmbt-footer-0-row-content">
			<div class="_rmbt-footer-0__col">
				<div class="block-details"> footer content</div>
			</div>
			<div class="_rmbt-footer-0__col">
				<div class="block-details"> footer content</div>
			</div>
			<div class="_rmbt-footer-0__col">
				<div class="block-details"> footer content</div>
			</div>
		</div>


		<div class="_rmbt-footer-0__row _rmbt-footer-0-row-copyright">
			<?php get_template_part( 'pages\_templates\blocks\copyright_block-0\copyright_block-0', '0' ); ?>

		</div>

	</section>
</div>
