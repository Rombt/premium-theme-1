						...
						<div class="rmbt-slide-in-tabs" data-rmbt-tabs-container="rmbt-contacts-tabs">
							...
							<?php if ( $qty > 0 ) { ?>
								<div class="rmbt-slide-in-tabs-content">
									<?php for ( $i = 0; $i < $qty; $i++ ) { ?>
										<div class="rmbt-slide-in-tabs-content__item <?php echo ( $i == 0 ) ? 'active' : ''; ?>" data-rmbt-tab-content-item="tab_<?php echo $i; ?>">
											...
											<a href="tel:<?php echo rmbt_phone_number_clear_redux( $rmbt_theme_options['rmbt-managers_phone'][ $i ] ); ?>" class="rmbt-contacts__phone subtitle-block"><?php echo esc_html( $rmbt_theme_options['rmbt-managers_phone'][ $i ] ); ?></a>
											<a href="mailto:<?php echo $rmbt_theme_options['rmbt-managers_email'][ $i ]; ?>" class="rmbt-contacts__email subtitle-block"><?php echo esc_html( $rmbt_theme_options['rmbt-managers_email'][ $i ] ); ?></a>
										</div>
									<?php } ?>
								</div>
								<div class="rmbt-slide-in-tabs-nav">
									<?php for ( $i = 0; $i < $qty; $i++ ) { ?>
										<div class="rmbt-slide-in-tabs-nav__item shadow-box <?php echo ( $i == 0 ) ? 'active' : ''; ?>" data-rmbt-tab-nav-item="tab_<?php echo $i; ?>">
